...
describe('ReserveSheet — 제약은 고를 수 없게 건다 (student-room.md §화면이 거는 제약)', () => {
  it('칩 8개 = 창, 첫 칩은 오늘, 빈 구간이 있는 첫 날이 골라져 있다', () => {
    ...
    // 네이티브 라디오 — 한 탭 멈춤, 화살표로 옮긴다 (Global Constraints: 선택은 네이티브 입력)
    expect(w.get('.rs__chips').attributes('role')).toBe('radiogroup')
    const chips = w.findAll('label.rs__chip')
    expect(chips).toHaveLength(8)
    expect(chips[0].text()).toContain('오늘')
    const radios = w.findAll<HTMLInputElement>('input[name="date"]')
    expect(radios.map((r) => r.attributes('type'))).toEqual(Array(8).fill('radio'))
    expect(radios[0].element.checked).toBe(true)
    expect(radios[0].attributes('aria-label')).toBe('오늘 10월 23일 금')
    expect(radios[1].attributes('aria-label')).toBe('10월 24일 토')
  ...
  it('시작을 바꾸면 끝이 범위 밖일 때만 다시 잡는다', async () => {
    const w = mount(ReserveSheet, { props: base })
    await w.findAll('input[name="span"]')[1].setValue()
  ...
  it('다른 날 칩 — 그 날의 구간, 빈 날은 문장', async () => {
    const w = mount(ReserveSheet, { props: base })
    await w.findAll('input[name="date"]')[1].setValue()
    expect((w.findAll('input[name="date"]')[1].element as HTMLInputElement).checked).toBe(true)
    expect(w.get('.rs__empty').text()).toBe('이 날은 비어 있는 시간이 없어요')
    await w.findAll('input[name="date"]')[2].setValue()
    await w.findAll('input[name="span"]')[0].setValue()
  ...
  it('재조회로 고른 구간이 사라지면 선택이 풀리고, 고른 날·입력은 그대로', async () => {
    ...
    expect((w.findAll('input[name="date"]')[2].element as HTMLInputElement).checked).toBe(true)
    expect(
      w.findAll('input[name="span"]').some((r) => (r.element as HTMLInputElement).checked),
    ).toBe(false)

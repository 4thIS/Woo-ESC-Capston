...
├── lora/            # cw @ssenu — store 소비·전처리(유닛 분해·FILE 청킹)·워커(TXN·재시도·FIFO)·modem.py·TIME 스케줄러·업링크 파싱·fake_modem.py
...
- **`link/`와 `lora/`는 서로 import하지 않는다.** 둘 사이의 유일한 통로는 `store.py`(SQLite 파일 하나)다. 링크는 `put_job/cancel_job/pending_results/mark_uploaded/set_config/pending_uplinks`, 파이프라인은 `pick_next/update/add_subjobs/get_config/on_config_changed/put_uplink`만 쓴다.
...
- `lora/`: fake 모뎀으로 acked / no_ack / GAP / BUSY / FILE_MISSING / DUP / cad_busy, 유닛 분해(`unit=0` → sub-row 2개), FILE 청킹과 `FILE_MISSING(seq)` 재송, TXN 1~255 롤링, TIME 스케줄.

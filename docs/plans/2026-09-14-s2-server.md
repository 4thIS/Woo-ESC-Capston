...
**Interfaces:**
- Produces: spec §4.3의 `/api/schools|buildings|rooms…` 전부. 변경 응답은 `{"...", "outbox_ids": [...]}`. 오류: `ValueError`→400 `{"detail": str}`, `api.NotFound`→404.
...
def test_validation_errors(client):
    ...
    assert bad.status_code == 400 and "20" in bad.json()["detail"]

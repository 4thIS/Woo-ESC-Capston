...
- 가입 2단계: `signup(email)` → 메일 링크 → `verify(token, name, student_no, password)` 가 `users` 행 생성(`pending_approval`). 신청 단계엔 토큰 행만 — 선점 불가.
...
- 메일은 **핸들러가 `s.commit()` 한 뒤** `bg.add_task` (`send_after_commit`) — BackgroundTasks 는 get_db 커밋보다 먼저 돈다. 전역 발송 시간당 100통.
...
- 브루트포스: `signup`·`login`·`forgot` email당 분당 5회, 초과 429. 메모리 dict(창 지난 키 정리). `/api/lora/time` 전역 분당 1회.
...
`server/alembic/env.py` 의 `run_migrations_online` 에서 연결 직후 `connection.exec_driver_sql("PRAGMA foreign_keys=OFF")` 한 줄 — `batch_alter_table` 이 `modems` 를 재생성할 때 `buildings.modem_id` FK 가 막지 않게(마이그레이션 동안만; 앱 엔진은 `app.db` 가 별도로 ON).
...
**Interfaces:**
- Produces: `password.hash(pw: str) -> str`, `password.verify(pw: str, stored: str) -> bool`, `password.dummy_verify(pw: str) -> None`(없는 사용자 로그인의 시간 맞추기); `tokens.issue(s, email, purpose) -> str`(평문), `tokens.consume(s, plain, purpose) -> str | None`(email), `tokens.last_issued_at(s, email, purpose) -> datetime | None`, `tokens.invalidate_all(s, email)`, `tokens.jwt_encode(settings, user) -> str`, `tokens.jwt_decode(settings, token) -> dict`(실패 `jwt.InvalidTokenError`). `TOKEN_TTL = timedelta(minutes=5)`, `RESEND_INTERVAL = timedelta(seconds=60)`. 시각은 `tokens.utcnow`(테스트가 monkeypatch).
...
def test_global_mail_cap(monkeypatch, capsys):
    ratelimit.reset()
    monkeypatch.setattr(mailer, "MAIL_PER_HOUR", 2)
    for i in range(3):
        mailer.send(Settings(), f"a{i}@x.test", "s", "b")
    out = capsys.readouterr().out
    assert "a0@x.test" in out and "a1@x.test" in out and "a2@x.test" not in out
```
...
MAIL_PER_HOUR = 100  # 전역 — Gmail 일일 한도 보호 (S4a §3.4)


def send(settings: Settings, to: str, subject: str, body: str) -> None:
    if not ratelimit.check("mail:global", limit=MAIL_PER_HOUR, window_s=3600.0):
        log.warning("메일 전역 상한 초과 — 발송 생략 to=%s", to)
        return
...
_MAX_WINDOW = 3600.0  # 가장 긴 창(메일 전역) — 이보다 오래된 기록은 어느 키에도 필요 없다
_last_sweep = 0.0


def _sweep(now: float) -> None:
    """창이 지난 키를 지운다 — email 별 키가 무한히 쌓이지 않게. 최대 분당 1회."""
    global _last_sweep
    if now - _last_sweep < 60.0:
        return
    _last_sweep = now
    for k in [k for k, v in _hits.items() if not v or now - v[-1] >= _MAX_WINDOW or (not k.startswith("mail:") and now - v[-1] >= 60.0)]:
        del _hits[k]
...
- Produces: `deps.current_user -> User`, `deps.require_admin`, `deps.require_student`(모두 `Depends`), `deps.CurrentUser`, `deps.AdminUser`, `deps.StudentUser`; `S.EMAIL_RE`, `S.EmailIn(email)`, `S.VerifyIn(token, name, student_no, password)`, `S.LoginIn(email, password)`, `S.LoginOut(token, role, school_id, name)`, `S.ResetIn(token, password)`, `S.UserOut(email, school_id, role, status, name, student_no, created_at, approved_at)`; `auth.router.router`(prefix `/api/auth`), `auth.router.admin`(prefix `/api/admin`, T6 에서 채움), `auth.router.send_after_commit(bg, s, settings, to, subject, body)`.
...
def send_after_commit(bg: BackgroundTasks, s: Session, settings, to: str, subject: str, body: str) -> None:
    """커밋을 먼저 끝내고 발송을 예약한다 — 쓰기 락을 쥔 채 SMTP 를 돌리지 않게 (S4a §3.4)."""
    s.commit()
    bg.add_task(mailer.send, settings, to, subject, body)


@router.post("/signup", status_code=202)
def signup(body: S.EmailIn, request: Request, bg: BackgroundTasks, s: Session = _DB):
    _limit(f"signup:{body.email}")
    if _school_by_domain(s, body.email) is None:
        raise HTTPException(400, "학교 웹메일이 아닙니다")
    if s.get(User, body.email) is None and tokens.can_send(s, body.email, "verify"):
        st = request.app.state.settings
        subj, text = mailer.verify_mail(st, tokens.issue(s, body.email, "verify"))
        send_after_commit(bg, s, st, body.email, subj, text)
...
def verify(body: S.VerifyIn, s: Session = _DB):
    ...
    if email is None or s.get(User, email) is not None:
        raise HTTPException(400, LINK_INVALID)
    school = _school_by_domain(s, email)
    if school is None:  # 발급 뒤 학교 도메인이 바뀐 경우
        raise HTTPException(400, LINK_INVALID)
    if s.scalar(select(User.email).where(User.school_id == school.id, User.student_no == body.student_no)):
...
def forgot(body: S.EmailIn, request: Request, bg: BackgroundTasks, s: Session = _DB):
    ...
    if user is not None and user.status == "active" and tokens.can_send(s, user.email, "reset"):
        st = request.app.state.settings
        subj, text = mailer.reset_mail(st, tokens.issue(s, user.email, "reset"))
        send_after_commit(bg, s, st, user.email, subj, text)
    return {"status": "sent"}
...
verify 의 학번 중복은 `UniqueConstraint(school_id, student_no)` 도 막지만, 전역 IntegrityError 핸들러가 `"constraint violation"` 을 내므로 먼저 조회해 사람이 읽을 문구로 409.
...
def test_cli_create_update_school_and_admin_then_login(tmp_path):
    ...
    r = _cli(db, "update-school", "--id", "1", "--email-domain", "wsu2.ac.kr")
    assert r.returncode == 0, r.stderr
...
def approve(email: str, request: Request, bg: BackgroundTasks, admin_user: User = AdminUser, s: Session = _DB):
    ...
    send_after_commit(bg, s, request.app.state.settings, u.email, *mailer.decision_mail(True, None))
    return out
...
def reject(email: str, body: S.RejectIn, request: Request, bg: BackgroundTasks, admin_user: User = AdminUser, s: Session = _DB):
    ...
    send_after_commit(bg, s, request.app.state.settings, u.email, *mailer.decision_mail(False, body.reason))
    return out
...
def test_time_broadcast_rate_limited(client, school):
    ...
    assert client.post("/api/lora/time").status_code == 429  # 전역 분당 1회 (S4a §3.3)
...
@rest.post("/time")
def time_now():
    """TIME 방송은 api 가 모든 모뎀에 보낸다(학교 개념 없음). 반복 호출로 전 노드를 깨우지 않게 전역 분당 1회 (S4a §3.3)."""
    if not ratelimit.check("lora:time", limit=1, window_s=60.0):
        raise HTTPException(429, "TIME 방송은 분당 1회")
...
- **Type consistency:** `scope.get_scoped(s, model, id_, school_id)` T7·T8 일치. `tokens.issue/consume/last_issued_at/can_send/invalidate_all/jwt_encode/jwt_decode` T3 정의 = T5·T6 사용. `mailer.send(settings, to, subject, body)` T4 = T5·T6 `send_after_commit(bg, s, settings, to, subject, body)` 및 테스트 monkeypatch 시그니처 `(st, to, subj, body)`. `AdminUser`/`CurrentUser`/`StudentUser` T5 = T6·T7·T8. `S.RejectIn` T6. `S.SchoolPatch/BuildingPatch/RoomPatch` T2 = T7. conftest `client`(Bearer)·`client_raw`·`school`·`admin_hdr`·`other_admin_hdr` T7 정의; T5 의 임시 `client_raw`·로컬 `schools` 와 T6 `seed` 의 학교·관리자 줄은 T7 Step 1 에서 정리(명시).

...
**Interfaces:**
- Produces: `app.auth.models.User(email, school_id, role, status, name, student_no, pw_hash, token_version, created_at, approved_at, approved_by, reject_reason)`, `EmailToken(token_hash, email, purpose, expires_at, used_at)` (email 은 FK 아님), `School.email_domain: str | None`, `Room.reservable: bool`, `Modem.school_id: int | None`, `S.RoomIn.reservable: bool = False`, `S.RoomPatch`·`S.BuildingPatch`·`S.SchoolPatch`(전 필드 선택), `S.ModemOut.school_id: int | None`. `SchoolIn` 은 변경 없음(email_domain 은 CLI 전용 — 웹 스키마에 넣지 않는다).

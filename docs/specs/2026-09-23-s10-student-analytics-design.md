...
- 전이: `requested → approved | rejected | cancelled | expired`, `approved → cancelled`. 종료 상태(`rejected|cancelled|expired`)는 불변.
- **No-show**(파생, 컬럼 없음) = `status='approved'` · `requested_by IS NOT NULL` · 종료 시각 지남 · `checked_in_at IS NULL`.

### 2.2 시각 규칙

- DB 는 naive UTC(앱 관행). **"지금"·요일·"오늘"·운영 시간은 `SCHOOL_TZ = Asia/Seoul`** 로 변환해 판정(노드 `TZ=KST-9` 와 동일). `app/domain/clock.py`: `now_utc()`, `to_local(dt)`, `local_today()`, `local_day_bounds(date) -> (utc_start, utc_end)`.
...
테스트 벡터 8개(예약 우선·시험기간·수업 49분/50분·휴강·특강·대여·슬롯 밖·자정 경계)를 `tests/test_room_state.py` 에 고정한다. dh 의 `determineLayout` 확정 뒤 같은 벡터를 펌웨어 테스트에도 넣어 대조하는 것은 열린 결정(§9).
...
겹침 검사에 `requested` 도 포함 — 같은 시간에는 **선착순 1명만** 신청 가능. 관리자 승인 시 **겹침 재검사**(사이에 관리자가 직접 예약을 넣었을 수 있음).
...
| 2 | 승격 | `approved` · KST 날짜 ∈ [오늘, 오늘+7] · 같은 `(bld, room, "RESV_SET", payload.resv_id)` 가 `queued|dispatched` 에 없음 → `enqueue_resv_set`. (매일 창 전체를 훑어 누락 보정; 중복은 outbox 조회로 막음) |
| 3 | 실패 재동기 | 지난 24 h `failed` outbox 의 `(bld, room)` 마다 실패한 `type` 의 kind 집합으로 `enqueue_full_sync(bld, room, kinds)` 1회 (`CMD`·`SET_ROOM`·`TIME` 은 제외) |
| 4 | 정리 | `reservations.date < 오늘−90` 삭제(id 반환) · `job_runs.ran_at < 오늘−90` 삭제 · `email_tokens.expires_at < 지금−7일` 삭제 |
...
- 트리거: `main.py` lifespan 의 `daily_loop(interval_s=60)` — 매 tick KST 시각이 `DAILY_HOUR_LOCAL` 이상이고 오늘 `job_runs(name="daily")` 가 없으면 실행. 서버가 04:00 에 꺼져 있었으면 다음 기동 첫 tick 에 실행.
...
- 학생 API 의 outbox 생성은 **취소 시 `RESV_DEL`** 하나뿐. 승인(관리자)·승격(일일 작업)이 `RESV_SET`.
...
| `POST /rooms/{id}/reservations` | `{date, s_h, s_m, e_h, e_m, subject}` | 201 `ResvMineOut` (`requested`, 서버 채번 — S4b `_free_id`) |
| `POST /me/reservations/{id}/cancel` | — | `ResvMineOut`. `requested` → `cancelled`; `approved` 는 시작 전만 → `cancelled` + `RESV_DEL`. 시작 후 409 |
...
- 성능 스모크(수동): 방 100·outbox 5000·예약 2000 에서 `analytics/allocation` 30일 < 200 ms.

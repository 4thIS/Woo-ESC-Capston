...
- 생성일시: 2026-09-23
- 수정일시: 2026-09-23 (r2 — PR #38 cw 리뷰 반영: 가입 2단계화(비밀번호는 링크 확인 뒤), 이메일 형식 검증, 커밋 뒤 메일, 스코프 구멍 4개, token_version, bld 전역 유일, PATCH 부분 수정, 노출 면적 보강) (r3 — 재검토: 학번 부분 유일·거절 뒤 재신청, 링크 5분+입력 30분, 메일 종류별 상한·도메인 상한, 레이트리밋 키별 창, verify 학번 조회 제한, `/lora/time` 10분)

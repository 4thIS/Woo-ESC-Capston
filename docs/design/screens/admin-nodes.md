...
`warnings[]` 에 배터리 경고가 들어 있으면 그 값을 `room.busy` 틴트 배지로 감싼다. **임계값은 서버가 판단하고 화면은 배열만 읽는다.**
...
`NodeOut`(S4b)에 화면이 쓰는 값이 **전부 들어 있다.** `warnings[]` 까지 서버가 판정해 주므로 화면이 임계값을 갖지 않는다. `StatusOut` 은 원자료가 필요할 때만 본다.
...
- **빈 상태**: 모뎀Pi 0개 — "등록된 모뎀Pi가 없습니다" + `모뎀Pi 등록`. 노드 표는 **강의실이 0곳일 때만** 비는다 — `NodeOut` 은 기대 노드(rooms × units)를 LEFT JOIN 하므로 방이 있으면 단말이 없어도 `unseen` 행이 나온다. 그때 문구는 "이 건물에 강의실이 없습니다" + [건물 · 강의실](admin-master.md)로 가는 링크. 방은 있는데 전부 `unseen` 이면 빈 상태가 아니라 정상 표이고, 아래 `등록 대기`가 다음 할 일이다.
